 ...
 *
 *  WjrGatewayExceptionHandler代码参考DefaultErrorWebExceptionHandler类的源码实现。
 *  在getHttpStatus方法中，定义了状态码为500（可以通过不同的异常类型来返回不同的状态码）；
 *  在getErrorAttributes方法中，我们根据异常的类型或者异常的错误信息来归类，构建了响应的异常信息对象。
 */
@Slf4j
public class WjrGatewayExceptionHandler extends DefaultErrorWebExceptionHandler {

    public WjrGatewayExceptionHandler(ErrorAttributes errorAttributes, ResourceProperties resourceProperties,
