...
import com.auth.properties.WjrValidateCodeProperties;
...
/**
 * 1、createCaptcha方法通过验证码配置文件WjrValidateCodeProperties生成相应的验证码，比如PNG格式的或者GIF格式的，
 ...
 * （Redis Key为wjr.captcha. + 客户端上送的key值，有效时间为配置文件定义的120秒），并将验证码图片以流的形式返回给客户端。
 *
...
public class ValidateCodeService {
    ...
    public void create(HttpServletRequest request, HttpServletResponse response) throws IOException, ValidateCodeException {
        ...
        WjrValidateCodeProperties code = properties.getCode();
        setHeader(response, code.getType());

        Captcha captcha = createCaptcha(code);
        redisService.set(WjrConstant.CODE_PREFIX + key, StringUtils.lowerCase(captcha.text()), code.getTime());
    ...
    public void check(String key, String value) throws ValidateCodeException {
        Object codeInRedis = redisService.get(WjrConstant.CODE_PREFIX + key);
    ...
    private Captcha createCaptcha(WjrValidateCodeProperties code) {
        Captcha captcha = null;
        if (StringUtils.equalsIgnoreCase(code.getType(), WjrConstant.GIF)) {
    ...
    private void setHeader(HttpServletResponse response, String type) {
        if (StringUtils.equalsIgnoreCase(type, WjrConstant.GIF)) {
